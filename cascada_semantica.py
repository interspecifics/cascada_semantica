...
from oraculo_generativo import gpt_transform_piece
...
def waterfall_display(body_instruction):
    ...
    for line in lines:
        # Three-dot animation
        for _ in range(random.randint(5, 30)):  # Randomly long loop
...
def cyberpunk_display(results, query):
    for result in results:
        text = result.page_content.replace('- ','')
        transformed_text = gpt_transform_piece(text, query)
...
def main():
    ...
    while True:
        ...
        display_thread = threading.Thread(target=cyberpunk_display, args=(results, query,))
